...
*
*   @method isTrytes
*   @param {string | array} trytes
*   @param {integer} length optional
*   @returns {boolean}
**/
var isTrytes = function(trytes, length) {

    // If an array, check if array items are numeric
    if( Array.isArray(trytes)){
       return trytes.length && trytes.filter(function(i){
         return isNaN(i);
       }).length === 0 && (!length || trytes.length === length)
    }
