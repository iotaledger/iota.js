...
function toTrytes(inputString) {
    ...
    for (var i = 0; i < inputString.length; i++) {
        ...
        // If not recognizable ASCII character throw
        if (asciiValue > 255) { throw new Error("Invalid input string"); }
...
function fromTrytes(inputTrytes) {
    ...
    if ( inputTrytes.length % 2 ) { throw new Error("Invalid input string length"); }
    ...
    for (var i = 0; i < inputTrytes.length; i += 2) {
        ...
        var char = inputTrytes[i];
        if( char < "A" || char > "Z" ) { 
            if( char !== "9" ) { throw new Error("Invalid char in input string"); }
            firstValue = 0;
        } else {
            firstValue = char.charCodeAt(0) - 64;
        }

        char = inputTrytes[i + 1];
        if( char < "A" || char > "Z" ) { 
            if( char !== "9" ) { throw new Error("Invalid char in input string"); }
            secondValue = 0;
        } else {
            secondValue = char.charCodeAt(0) - 64;
        }

        outputString += String.fromCharCode(firstValue + secondValue * 27);
    }
