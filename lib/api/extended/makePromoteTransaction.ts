...
import { Callback, CurlFunction, Maybe, Transaction, Transfer } from '../types'
import { isPromotable, makeSendTransfer } from './index'
 ...
 */
export const makePromoteTransaction = (curl: CurlFunction) => (
    ...
    return Promise.resolve(validate(hashValidator(tailTransaction), transferArrayValidator(spamTransfers)))
        ...
        .then(ok => {
            ...
            return makeSendTransfer(curl)(spamTransfers[0].address, depth, minWeightMagnitude, spamTransfers, {
        ...
        .then(async (transactions: Transaction[]) => {
            if (
                ...
                setTimeout(() => {
                    makePromoteTransaction(curl)(tailTransaction, depth, minWeightMagnitude, spamTransfers, {
                        delay,
                        interrupt,
                    })
                }, delay)
