...
import { Callback, Hash, Transaction, Trytes } from '../types'
import { findTransactionObjects, getLatestInclusion } from './index'
 ...
 * @param transactions
 */
const withInclusionState = (transactions: Transaction[]) =>
    getLatestInclusion(transactions.map(tx => tx.hash)).then(states =>
    ...
    return (
        Promise.try(() => {
            ...
            .then(spendingTransactions => {
                ...
                return (
                    withInclusionState(spendingTransactions)
