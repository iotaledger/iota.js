...
*
    * @returns 
*/
api.prototype.promoteTransaction = function(tail, depth, minWeightMagnitude, transfer, delay, maximum, callback) {
    var self = this;
    ...
    if (maximum <= 0) goto promote_end;
    self.sendTransfer(transfer[0].address, depth, minWeightMagnitude, transfer, {reference: tail}, function(err, res) {
        if (err == null && delay > 0 && maximum > 1) {
            setTimeout (function() {
                self.promoteTransaction(tail, depth, minWeightMagnitude, transfer, delay, maximum - 1, callback);
            }, delay);
        } else {
...
/*
 * Should tell us if the subtangle below {tail} resolves to a consistent ledger
 */
api.prototype.isTailConsistent(tail, callback) {
    callback(null, true);
}

/*
 * Wraps {isTailConsistent} in a promise so that its value is returned
 */
api.prototype.isPromotable = function(tail) {
    var self = this;
    return (new Promise(function(res, rej) {
        self.isTailConsistent(tail, function(err, isConsistent) {
            res(isConsistent);
        });
    })).then(function(val) {
        return val;
