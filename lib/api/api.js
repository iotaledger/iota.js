...
*   @returns {object} success
**/
api.prototype.findTransactionObjects(input, callback) {
...
api.prototype.getAccountData = function(seed, options, callback) {
    ...
    //  @sent all sent transfers
    //  @received all received transfers
    ...
    self.getNewAddress(seed, addressOptions, function(error, addresses) {
        ...
        valuesToReturn.addresses.push(addresses.slice(0, -1));
        ...
        self._bundlesFromAddresses(addresses, true, function(error, bundles) {
            ...
            // update the confirmed balance from transactions that were confirmed
            ...
            bundles.forEach(function(bundle) {
                ...
                bundle.forEach(function(bundleEntry, bundleIndex) {
                    ...
                    if (valuesToReturn.addresses.indexOf(bundleEntry.address) > -1) {

                        // Add to balance if transaction is confirmed
                        if (bundleEntry.persistence) {

                            valuesToReturn.balance += bundleEntry.value;
                        }

                        // Check if it's a remainder address
                        var isRemainder = bundleEntry.currentIndex === bundleEntry.lastIndex;

                        // check if sent transaction
                        if (balance < 0 && !spentAlreadyAdded && !isRemainder) {

                            valuesToReturn.sent.push(bundle);
                        ...
                        else if (balance >= 0 && !spentAlreadyAdded && !isRemainder) {

                            valuesToReturn.received.push(bundle);
                        }
                    }
                })
            })
            ...
            // add all transfers
            valuesToReturn.all.push(bundles);
