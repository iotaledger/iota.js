...
*   @param {String || Array || Object} inputAddresses - if String or Array, 
*                                      tailHashes     - if Object like: { hashes: ['TAIL...9999'] }
*
*   @returns {Bool || Array} Bool  - if called on inputs
*                            Array - if called on tail hashes
**/
api.prototype.isReattachable = function(inputAddresses, callback) {

    var self = this;

    if (arguments[0].hasOwnProperty('hashes')) {

      var hashes = arguments[0].hashes;

      for (var i = 0; i < hashes.length; i++) {

        if (!inputValidator.isTrytes(hashes[i])) {
          return callback(errors.invalidInputs());
        }
      }

      return async.map(hashes, function(hash, cb) {

        self.getBundle(hash, function(err, bundle) {

          if (err) {
            return cb(err);
          }

          var inputs = bundle.filter(function(tx) { return tx.value < 0 }).map(function(tx) { return tx.address });
          var outputs = bundle.filter(function(tx) { return tx.value > 0 }).reduce(function(sum, tx) { return sum + tx.value }, 0);

          self.getBalances(inputs, 100, function(err, balances) {

            if (err) {
              return cb(err);
            }

            cb(null, balances.balances.reduce(function(sum, balance) { return sum + parseInt(balance) }, 0) >= outputs);
          })
        })
      }, function(err, res) {
