import {
    ...
    FindTransactionsSearchKeys,
    FindTransactionsSearchValues,
    findTxValidSearchKeys,
    GetBalancesCommand,
 ...
 *   @returns {object} command
 **/
export const findTransactions = (searchValues: FindTransactionsSearchValues): FindTransactionsCommand => {
    const command: FindTransactionsCommand = {
        command: IRICommand.FIND_TRANSACTIONS,
    }

    const searchKeys = Object.keys(searchValues) as FindTransactionsSearchKeys

    searchKeys.forEach(key => {
        if (findTxValidSearchKeys.indexOf(key) > -1) {
 ...
 *   @returns {object} command
 **/
export const getNodeInfo = (): GetNodeInfoCommand => ({ command: IRICommand.GET_NODE_INFO })

/**
 *   @method getNeighbors
 *   @returns {object} command
 **/
export const getNeighbors = (): GetNeighborsCommand => ({ command: IRICommand.GET_NEIGHBORS })
