...
Multisig.prototype.addSignature = function(bundleToSign, cosignerIndex, inputAddress, key, callback) {
    ...
    for (var i = 0; i < bundle.bundle.length; i++) {
        ...
        if (bundle.bundle[i].value < 0 && bundle.bundle[i].address === inputAddress) {
            ...
            // Case 1:
            //  it's the first person in the multisig providing the signature
            if (cosignerIndex === 0) {
                ...
                for (var j = 1; j < security; j++) {
                    ...
                    bundle.addEntry(1, bundle.bundle[i].address, 0, bundle.bundle[i].tag, bundle.bundle[i].timestamp, i + j);
                ...
                // calculate the current number of signed transactions
                // use that for the bundle hash calculation
            }
...
*   @returns {bool}
**/
Multisig.prototype.validateSignatures = function(signedBundle, inputAddress) {
    ...
    for (var i = 0; i < signedBundle.length; i++) {
        ...
        if (signedBundle[i].value < 0 && signedBundle[i].address === inputAddress) {
            ...
            while (signedBundle[i])

            for (var j = i; j < signedBundle.length; j++) {

                // if we reached remainder bundle
                if (inputValidator.isNinesTrytes(signedBundle[j].signatureMessageFragment)) {
                    break;
                }

                signatureFragments.push(signedBundle[j].signatureMessageFragment)
            }
