...
import { IAliasUnlockBlock, ALIAS_UNLOCK_BLOCK_TYPE } from "../../models/unlockBlocks/IAliasUnlockBlock";
 ...
 */
export const MIN_ALIAS_UNLOCK_BLOCK_LENGTH: number = SMALL_TYPE_LENGTH + UINT16_SIZE;
 ...
 */
export function deserializeAliasUnlockBlock(readStream: ReadStream): IAliasUnlockBlock {
    if (!readStream.hasRemaining(MIN_ALIAS_UNLOCK_BLOCK_LENGTH)) {
        throw new Error(
            `Alias Unlock Block data is ${readStream.length()} in length which is less than the minimimum size required of ${MIN_ALIAS_UNLOCK_BLOCK_LENGTH}`
        );
    }
