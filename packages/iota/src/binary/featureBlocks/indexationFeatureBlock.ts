 ...
 */
export function deserializeIndexationFeatureBlock(readStream: ReadStream): IIndexationFeatureBlock {
    if (!readStream.hasRemaining(MIN_INDEXATION_FEATURE_BLOCK_LENGTH)) {
        throw new Error(
            `Indexation Feature Block data is ${readStream.length()} in length which is less than the minimimum size required of ${MIN_INDEXATION_FEATURE_BLOCK_LENGTH}`
        );
    }
