...
test('getKeysToBatch() should return correct keys.', t => {
    t.deepEqual(
        getKeysToBatch(<BatchableCommand<FindTransactionsCommand>>command, BATCH_SIZE),
        ['tags', 'approvees']
    )
})
...
test('getKeysToBatch() should return no empty array for non-batchable keys.', t => {
    t.deepEqual(
        getKeysToBatch(<BatchableCommand<FindTransactionsCommand>>commandWithoutBatchableKeys, BATCH_SIZE),
        []
    )
})