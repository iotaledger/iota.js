...
export class Converter {
  ...
  static bytesToUtf8(array, startIndex, length) {
    ...
    for (let i = start; i < start + len; i++) {
      ...
      if (value < 0x80) {
        ...
        // surrogate pair
        const charCode = ((value & 0x07) << 18 | (array[i + 1] & 0x3F) << 12 | (array[i + 2] & 0x3F) << 6 | array[i + 3] & 0x3F) - 0x010000;
  ...
  static hexToBytes(hex, reverse) {
    ...
    if (Converter.DECODE_LOOKUP) {
      ...
      while (i < length) {
        array[n++] = Converter.DECODE_LOOKUP[hex.charCodeAt(i++)] << 4 | Converter.DECODE_LOOKUP[hex.charCodeAt(i++)];
      }
