...
export class UnitsHelper {
  ...
  static formatUnits(value, unit, decimalPlaces = 2) {
    ...
    return unit === "i" ? `${value} i` : `${UnitsHelper.convertUnits(value, "i", unit).toFixed(decimalPlaces)} ${unit}`;
  }
  /**
   * Format the value in the best units.
   * @param value The value to format.
  ...
  static convertUnits(value, fromUnit, toUnit) {
    ...
    const numDecimals = UnitsHelper.UNIT_MAP[toUnit].dp; // We cant use toFixed to just convert the new value to a string with
    ...
    parts[1] = parts[1].slice(0, numDecimals); // Finally join the parts and convert back to a real number
...
//# sourceMappingURL=unitsHelper.mjs.map
