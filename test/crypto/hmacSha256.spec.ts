...
describe("HmacSha256", () => {
    test("Can perform a hmac on short ascii", () => {
        const hmacSha256 = new HmacSha256(Converter.asciiToBytes("mykey"));
        hmacSha256.update(Converter.asciiToBytes("abc"));
    ...
    test("Can perform a hmac on empty ascii", () => {
        const hmacSha256 = new HmacSha256(Converter.asciiToBytes("mykey"));
        hmacSha256.update(Converter.asciiToBytes(""));
        const digest2 = hmacSha256.digest();
        expect(Converter.bytesToHex(digest2))
    ...
    test("Can perform a hmac on sentence", () => {
        const hmacSha256 = new HmacSha256(Converter.asciiToBytes("mykey"));
        hmacSha256.update(Converter.asciiToBytes("The quick brown fox jumps over the lazy dog"));
        const digest2 = hmacSha256.digest();
        expect(Converter.bytesToHex(digest2))
