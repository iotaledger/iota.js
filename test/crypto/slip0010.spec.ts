...
describe("Slip0010", () => {
    ...
    test("Can generate a key and chain code from seed and path m/0H", () => {
        const { privateKey, chainCode } = Slip0010.derivePath(
            Converter.hexToBytes("000102030405060708090a0b0c0d0e0f"), new Bip32Path("m/0"));
        expect(Converter.bytesToHex(privateKey))
    ...
    test("Can generate a key and chain code from seed and path m/0H/1H", () => {
        const { privateKey, chainCode } = Slip0010.derivePath(
            Converter.hexToBytes("000102030405060708090a0b0c0d0e0f"), new Bip32Path("m/0/1"));
        expect(Converter.bytesToHex(privateKey))
    ...
    test("Can generate a key and chain code from seed and path m/0H/1H/2H", () => {
        const { privateKey, chainCode } = Slip0010.derivePath(
            Converter.hexToBytes("000102030405060708090a0b0c0d0e0f"), new Bip32Path("m/0/1/2"));
        expect(Converter.bytesToHex(privateKey))
    ...
    test("Can generate a key and chain code from seed and path m/0H/1H/2H/2H", () => {
        const { privateKey, chainCode } = Slip0010.derivePath(
            Converter.hexToBytes("000102030405060708090a0b0c0d0e0f"), new Bip32Path("m/0/1/2/2"));
        expect(Converter.bytesToHex(privateKey))
            .toEqual("30d1dc7e5fc04c31219ab25a27ae00b50f6fd66622f6e9c913253d6511d1e662");
        expect(Converter.bytesToHex(chainCode))
            .toEqual("8f6d87f93d750e0efccda017d662a1b31a266e4a6f5993b15f5c1f07f74dd5cc");

        const publicKey = Slip0010.getPublicKey(privateKey);
        expect(Converter.bytesToHex(publicKey))
            .toEqual("008abae2d66361c879b900d204ad2cc4984fa2aa344dd7ddc46007329ac76c429c");
    });

    test("Can generate a key and chain code from seed and path m/0H/1H/2H/2H/1000000000H", () => {
        const { privateKey, chainCode } = Slip0010.derivePath(
            Converter.hexToBytes("000102030405060708090a0b0c0d0e0f"), new Bip32Path("m/0/1/2/2/1000000000H"));
        expect(Converter.bytesToHex(privateKey))
            .toEqual("8f94d394a8e8fd6b1bc2f3f49f5c47e385281d5c17e65324b0f62483e37e8793");
        expect(Converter.bytesToHex(chainCode))
            .toEqual("68789923a0cac2cd5a29172a475fe9e0fb14cd6adb5ad98a3fa70333e7afa230");

        const publicKey = Slip0010.getPublicKey(privateKey);
        expect(Converter.bytesToHex(publicKey))
            .toEqual("003c24da049451555d51a7014a37337aa4e12d41e485abccfa46b47dfb2af54b7a");
    });
});
