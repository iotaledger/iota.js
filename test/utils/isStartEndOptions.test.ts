...
import { isStartEndOptions } from '../../lib/utils'

test('isStartEndOptions() returns true for valid options.', t => {
    t.is(
        isStartEndOptions({ start: 2, end: 10 }),
        true,
        'isStartEndOptions() should return true for valid options.'
    )

    t.is(
        isStartEndOptions({ start: 2, end: 2 }),
        true,
        'isStartEndOptions() should return true for valid options.'
    )
})
...
test('isStartEndOptions() returns false for invalid end option.', t => {
    t.is(
        isStartEndOptions({ start: 2, end: 1 }),
        false,
        'isStartEndOptions() should return false if start > end' 
    )
})
